...
Everything except Opportunity Radar below is deployed and live in production (see "Production Deployment") with no known bugs or unfinished work. Opportunity Radar (see its sections below, most recently "Opportunity Radar: review fixes") is implemented, locally verified end to end against a real Postgres database and the actual UI through the "split into ActiveProject / BusinessProspect" checkpoint, but **uncommitted and unmigrated in production** - it is staying local for Jonathan's review per the root `CLAUDE.md`'s review-before-committing rule. The most recent checkpoint's code changes are unit-tested and build-verified but not yet re-verified end to end against a live Postgres database and browser (see that section for why) - do that pass before considering it settled. Before that, the shared "Line" homepage game: `LineController` (see its own doc comment for the anonymous-write design rationale), the `LineKindProgress` entity/migration, and the admin reset endpoint on `AdminController` - see "Three controllers, three trust levels" below for where it fits. This file and the UI repo's `CLAUDE.md` are both kept current - read both before resuming.
